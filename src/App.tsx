...
import {
  ...
  DAILY_FORTUNE_SHARE_TEXT,
  shouldRevealFortune,
} from './lib/scratchCard';

const App = () => {
  const canvasRef = useRef<HTMLCanvasElement>(null);
  const [instructionText, setInstructionText] = useState('Use your finger to scratch the gold area!');
  ...
  useEffect(() => {
    ...
    const checkScratched = () => {
      ...
      const scratchedPercent = calculateScratchedPercent(imageData.data);

      if (shouldRevealFortune(scratchedPercent)) {
        canvas.style.transition = 'opacity 0.5s ease-out';
        canvas.style.opacity = '0';
        setInstructionText('Fortune Revealed!');
        setTimeout(() => {
